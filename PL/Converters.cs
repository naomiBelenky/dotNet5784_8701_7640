...
class ConvertStatusToColor : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        if ((((BO.Task)value).PlanToFinish < BlApi.Factory.Get().Clock) && ((BO.Task)value).Status != BO.Status.Done) return "OrangeRed";
        if (((BO.Task)value).Status == BO.Status.Scheduled) return "Gray";
        if (((BO.Task)value).Status == BO.Status.OnTrack) return "Orange";
        if (((BO.Task)value).Status == BO.Status.Done) return "LimeGreen";
        return "true";
    }
